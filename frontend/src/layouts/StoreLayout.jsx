...
export default function StoreLayout() {
  ...
  function submitSearch(e) {
    e.preventDefault();
    if (searchQ.trim()) navigate(`/tim-kiem?q=${encodeURIComponent(searchQ.trim())}`);
  }
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <header className="store-header">
        <div className="container store-header-inner">
          ...
          <nav className="store-nav" aria-label="Điều hướng chính">
            <NavLink to="/san-pham">Tất cả</NavLink>
            <NavLink to="/san-pham/ao-so-mi">Áo sơ mi</NavLink>
            <NavLink to="/san-pham/ao-khoac">Áo khoác</NavLink>
            <NavLink to="/san-pham/quan-jean">Quần jean</NavLink>
            <NavLink to="/san-pham/vay-dam" className="nav-sale">Váy/Đầm</NavLink>
          </nav>
          ...
          <div className="store-actions">
            {user?.role === 'CUSTOMER' ? (
              <>
                <Link to="/tai-khoan" className="icon-link" aria-label="Tài khoản của tôi">
                  <span className="account-avatar" aria-hidden="true">
                    {(user.fullName || user.email || '?').trim().charAt(0).toUpperCase()}
                  </span>
                </Link>
                <Link to="/yeu-thich" className="icon-link" aria-label="Sản phẩm yêu thích"><IconHeart /></Link>
                <Link to="/don-hang-cua-toi" className="icon-link" aria-label="Đơn hàng của tôi"><IconReceipt /></Link>
                <Link to="/gio-hang" className="icon-link" aria-label="Giỏ hàng">
                  <IconBag />
              ...
              <>
                ...
                <Link to="/dang-nhap-nhan-vien" className="staff-link">Nhân viên</Link>
              </>
            )}
          </div>
        </div>
      ...
      <footer className="store-footer">
        <div className="container">
          ...
          <div>
            <Link to="/ho-tro">Trung tâm hỗ trợ</Link>
            {' · '}
